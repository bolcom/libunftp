...
use std::{
    collections::HashMap, convert::TryInto, fs, num::NonZeroU32, path::Path, time::Duration,
};
...
impl JsonFileAuthenticator {
    ...
    ) -> Result<Option<IpRange<Ipv4Net>>, String> {
        ip_ranges
            .map(|v| {
                let range: Result<IpRange<Ipv4Net>, _> = v
                    ...
                    .map(|s| {
                        s.parse::<Ipv4Net>()
                            .map_err(|_| format!("could not parse IP ranges for user {}", username))
                    })
                    .collect();
                range
            })
...
mod test {
  ...
  },
  {
    ...
    "allowed_ip_ranges": ["127.0.0.1/8"]
        ...
        assert_eq!(
            json_authenticator
                .authenticate("carol", &"not so secure".into())
                .await
                .unwrap(),
            DefaultUser
        );
        assert_eq!(
            json_authenticator
                .authenticate("dan", &"".into())
                .await
                .unwrap(),
            DefaultUser
        );
        match json_authenticator
            .authenticate("carol", &"this is the wrong password".into())
            .await
        {
        ...
        match json_authenticator
            .authenticate("bella", &"this is the wrong password".into())
            .await
        {
            Err(AuthenticationError::BadPassword) => assert!(true),
            _ => assert!(false),
        ...
        match json_authenticator
            .authenticate("chuck", &"12345678".into())
            .await
        {
  ...
  },
  {
    ...
    async fn test_json_cert_authenticate() {
        ...
        let cert: &[u8] = &[
            ,
            ...
            ,
        ];
